...
function [] = gen_coder_fcn_inline(module_name,input_nodes,circuit_nodes,output_nodes,symmetricity)
    ...
    if ~isempty(regs)
        ...
        for i = 2:length(input_nodes)
            fprintf(f_fcn,sprintf(' \t%s_reg=%d',input_nodes(i).name,0));
            fprintf(f_fcn,';\n');
        end
        for i = 1:length(regs)
            for j = 1:regs{i}.reg_cnt
