...
function [] = gen_coder_fcn_me(module_name,input_nodes,circuit_nodes,output_nodes)
    ...
    if ~isempty(regs)
        ...
        %me, coeff_reg
        for i = 2:length(input_nodes)
            fprintf(f_fcn,sprintf(' %s_reg',input_nodes(i).name));
        end
        ...
        %fprintf(f_fcn,'\t\t');
        %me,initialyzing coeff_regs
        for i = 2:length(input_nodes)
            fprintf(f_fcn,sprintf(' \t%s_reg= 0',input_nodes(i).name));
            fprintf(f_fcn,';\n');
        end
        for i = 1:length(regs)
            for j = 1:regs{i}.reg_cnt
    ...
    for i = 1:length(circuit_nodes)
        ...
        if circuit_nodes(i_node).is_inline
            ...
            for j = 1:length(node_equations{i_node}.inputs)
                if node_equations{i_node}.inputs{j}(1)=='h'
                    curr_eq = sprintf('%s%s_reg,',curr_eq,node_equations{i_node}.inputs{j});
                else
...
%                         case 'odd_symmetric'
%                             curr_eq = sprintf('%s);\n',curr_eq);
% %                             if i <=(length(input_nodes)-2)+1
% %                     
% %                                 curr_eq = sprintf('%s_reg);\n',curr_eq);
% %                             else
% %                                 curr_eq = sprintf('%s);\n',curr_eq);
% %                             end
%                         otherwise
%                             if i <=length(input_nodes)-1
%                     
%                                 curr_eq = sprintf('%s_reg);\n',curr_eq);
%                             else
%                                 curr_eq = sprintf('%s);\n',curr_eq);
...
%                     end
    ...
    for i = 2:length(input_nodes)
            fprintf(f_fcn,sprintf(' \t%s_reg=%s',input_nodes(i).name,input_nodes(i).name));
            fprintf(f_fcn,';\n');
    end
    fprintf(f_fcn,'end');
    fclose(f_fcn);
