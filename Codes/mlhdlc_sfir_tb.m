...
y_orj=conv(x_in,Coeffs)';
y_orj = y_orj(10:end);
len = length(x_in);
y_out = zeros(length(x),1);
y_8out = zeros(1,len);
y_10out = zeros(1,len);
y_14out = zeros(1,len);
...
for i=1:length(x)
    ...
    [y_out(i)] = mlhdlc_sfir(data, Coeff);
end
...
for ii=1:len
    ...
    % call to the design 'mlhdlc_sfir' that is targeted for hardware
    [y_8out(ii)] = mlhdlc_sfir(data, Coeffs_8quantized);
end
...
for ii=1:len
    ...
    % call to the design 'mlhdlc_sfir' that is targeted for hardware
    [y_10out(ii)] = mlhdlc_sfir(data, Coeffs_10quantized);
end
...
for ii=1:len
    ...
    % call to the design 'mlhdlc_sfir' that is targeted for hardware
    [y_14out(ii)] = mlhdlc_sfir(data, Coeffs_14quantized);
end
