function [] = gen_coder_fcn_changed(module_name,input_nodes,circuit_nodes,output_nodes,coeffs)
    ...
    if ~isempty(regs)
        ...
        for i = 1:length(coeffs)
            fprintf(f_fcn,sprintf(' %s_reg',coeffs(i).name));
        end
        ...
        %me
        for i = 1:length(coeffs)
            fprintf(f_fcn,sprintf(' \t%s_reg=%d',coeffs(i).name,0));
            fprintf(f_fcn,';\n');
        end
        for i = 1:length(regs)
            for j = 1:regs{i}.reg_cnt
    ...
    for i = 1:length(coeffs)
            fprintf(f_fcn,sprintf(' \t%s_reg=%s',coeffs(i).name,coeffs(i).name));
            fprintf(f_fcn,';\n');
    end
        fprintf(f_fcn,'\n');
    fprintf(f_fcn,'end');
